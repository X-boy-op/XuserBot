...
from xbot import CMD_LIST, LOAD_PLUG, SUDO_LIST, bot
from xbot.Configs import Config
from xbot.wraptools import (
    am_i_admin,
    ignore_bot,
    ignore_fwd,
    ignore_grp,
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"xbot/modules/{shortname}.py")
        ...
        path = Path(f"xbot/modules/{shortname}.py")
        name = "xbot.modules.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        mod.bot = bot
        mod.tgbot = bot.tgbot
        ...
        sys.modules["xbot.util"] =  xbot.utils
        sys.modules["userbot.utils"] = xbot.utils
        sys.modules["userbot.plugins"] = xbot.modules
        mod.Config = Config
        ...
        sys.modules["xbot.events"] = xbot.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["xbot.modules." + shortname] = mod
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[Here](https://t.me/XsupportOT)"
    ...
    def decorator(func):
        ...
        async def wrapper(event):
            ...
            if sed.is_admin:
                ...
                await event.reply("Bhosdike make me admin first")
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"xbot/modules/assistant/{shortname}.py")
        ...
        path = Path(f"xbot/modules/assistant/{shortname}.py")
        name = "xbot.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        mod.tgbot = bot.tgbot
        ...
        spec.loader.exec_module(mod)
        sys.modules["xbot.modules.assistant" + shortname] = mod
